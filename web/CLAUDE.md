...
- `api.ts` — `apiFetch<T>` helper plus `createAuthenticatedFetch(useAppBridge())`, which fetches the session token via `.idToken()` and attaches it as a Bearer header (App Bridge 4 — the v3 `@shopify/app-bridge/utilities` `authenticatedFetch` is incompatible)
