 ...
 */
export async function getShopAccessToken(shopDomain: string, env: Env): Promise<string | null> {
  ...
  if (session.isExpired(EXPIRY_BUFFER_MS) && session.refreshToken) {
    try {
      ...
      console.log(`[getShopAccessToken] refreshed token for ${shopDomain}, expires=${refreshed.expires?.toISOString()}`);
      return refreshed.accessToken ?? null;
    } catch (err) {
      console.error(`[getShopAccessToken] refresh failed for ${shopDomain}:`, err);
