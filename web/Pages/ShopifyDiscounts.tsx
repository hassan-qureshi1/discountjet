...
import {
  Badge, Banner, BlockStack, Button, ButtonGroup, Card, IndexTable, InlineStack, Page, Spinner, Tabs, Text,
} from '@shopify/polaris';
import { useDiscountsQuery, useShopifyDiscountsQuery } from '../discounts/hooks';
import type { DiscountEngineKind, ShopifyDiscount } from '../types/discounts';
...
export default function ShopifyDiscounts() {
  ...
  const { data: shopifyData, isLoading, error } = useShopifyDiscountsQuery();
  const { data: appData } = useDiscountsQuery();

  const discounts = shopifyData?.shopifyDiscounts ?? [];
  const appDiscounts = appData?.discounts ?? [];
  ...
  const filters: { id: FilterId; label: string }[] = [
    ...
    { id: 'scheduled', label: 'Scheduled' }, { id: 'expired', label: 'Expired' },
  ];
  ...
  const onSelectEngine = (kind: DiscountEngineKind) => { setModalOpen(false); navigate(`/discounts/new?type=${kind}`); };
          ...
          <Tabs tabs={tabs} selected={selected} onSelect={setSelected}>
            {isLoading ? (
              ...
              <IndexTable
                ...
                selectable={false}
                headings={[
                  { title: 'Title' }, { title: 'Status' }, { title: 'Method' },
                  { title: 'Type' }, { title: 'Used', alignment: 'end' }, { title: '' },
                ]}
              >
                {rows.map((d, index) => {
                  const app = appOf(d);
                  return (
                    <IndexTable.Row id={d.id} key={d.id} position={index}>
                      ...
                      <IndexTable.Cell><Badge tone={d.status === 'Active' ? 'success' : undefined}>{d.status}</Badge></IndexTable.Cell>
                      <IndexTable.Cell><Text as="span" tone="subdued">{d.method}</Text></IndexTable.Cell>
                      <IndexTable.Cell>
                        {d.engine ? <Badge tone="magic">{d.type}</Badge> : <Text as="span" tone="subdued">{d.type}</Text>}
                      </IndexTable.Cell>
                      <IndexTable.Cell><Text as="span" numeric alignment="end">{d.used}</Text></IndexTable.Cell>
                      <IndexTable.Cell>
                        {d.appId ? (
                          <ButtonGroup>
                            <Button variant="plain" onClick={() => navigate(`/discounts/${d.appId}`)}>View</Button>
                            {!app?.campaignId && (
                              <Button variant="plain" onClick={() => navigate(`/discounts/${d.appId}/edit`)}>Edit</Button>
                            )}
                          </ButtonGroup>
                        ) : (
                          <Button variant="plain" disabled>View in Shopify</Button>
                        )}
                      </IndexTable.Cell>
                    </IndexTable.Row>
                  );
