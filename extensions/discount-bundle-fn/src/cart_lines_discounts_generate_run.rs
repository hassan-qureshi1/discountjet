use super::schema;
use discount_bundle::config::{parse_bundle_config, PriorityCode, SelectionStrategy};
use discount_bundle::engine::{build_candidates, DiscountValue, Line};
use discount_bundle::shared::{current_platform, id_from_gid, should_yield};
...
fn cart_lines_discounts_generate_run(
    ...
    let cfg = match parse_bundle_config(raw_cfg) {
        Some(c) => c,
    ...
    let output = build_candidates(&cfg, &lines, current);
    if output.candidates.is_empty() {
        return Ok(empty());
    }

    let selection_strategy = match output.strategy {
