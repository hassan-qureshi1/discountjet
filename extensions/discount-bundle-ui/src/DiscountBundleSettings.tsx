...
const METAFIELD_DEFINITION_NAME = "Buy X, get Y configuration";
...
// Wizard steps. The offer type is fixed (this is the Buy X, get Y function).
const STEP_LABELS = ["Products", "Discount & rules", "Review"] as const;

/** True once at least one offer has both qualifying and discounted products. */
function hasBundleData(bundles: Bundle[]): boolean {
  return bundles.some((b) => {
    try {
      const s = JSON.parse(b?.source_variants || "[]");
      const t = JSON.parse(b?.target_variants || "[]");
...
function App() {
  const initial = useMemo<BundleFormData>(() => {
  ...
  const [step, setStep] = useState(() => (hasBundleData(initial.bundles) ? lastStep : 0));
  const goNext = () => setStep((s) => Math.min(s + 1, lastStep));
  const goBack = () => setStep((s) => Math.max(s - 1, 0));

  const sizeBytes = useMemo(() => getMetafieldSizeBytes(formData), [formData]);
  const validationErrors = useMemo(() => validateBundleConfig(formData), [formData]);
  ...
  const patch = (p: Partial<BundleFormData>) => setFormData((prev) => ({ ...prev, ...p }));
  const updateBundle = (index: number, field: keyof Bundle, value: string | boolean) =>
    setFormData((prev) => ({
      ...prev,
      bundles: prev.bundles.map((b, i) => (i === index ? { ...b, [field]: value } : b)),
    }));
  const addBundle = () => setFormData((prev) => ({ ...prev, bundles: [...prev.bundles, newBundle()] }));
  const removeBundle = (index: number) =>
    setFormData((prev) => ({ ...prev, bundles: prev.bundles.filter((_, i) => i !== index) }));
  ...
  useEffect(() => {
    ...
    if (el && formData.bundles.length) {
      el.dispatchEvent?.(new Event("input", { bubbles: true }));
    }
  }, [formData]);

  const apply = async () => {
    const errors = validateBundleConfig(formData);
    if (errors.length > 0) throw new Error(errors.join(" "));
    const value = JSON.stringify(buildBundleConfig(formData));
          ...
          {step === 0 ? (
            <s-stack gap="base">
              <s-banner tone="warning">
                You can’t add the same discounted product to more than one offer here. To do that, create a
                separate discount — Shopify always applies the best one for the shopper.
              </s-banner>
              ...
              {formData.bundles.length > 0 ? (
                formData.bundles.map((bundle, index) => (
                  <BundleCard
                    ...
                    bundle={bundle}
                    bundleIndex={index}
                    allBundles={formData.bundles}
                    onUpdate={(field, value) => updateBundle(index, field, value)}
              ...
              <s-stack direction="inline" gap="base" alignItems="center">
                <s-button onClick={addBundle} variant="secondary">
                  Add another offer
                </s-button>
              </s-stack>
            </s-stack>
