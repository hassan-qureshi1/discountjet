...
describe('discount data layer', () => {
  it('fetchDiscounts requests /api/discounts and returns discounts + counts', async () => {
    const payload = {
      discounts: [{ id: '1', name: 'A', symbol: '%', type: 'Tier', status: 'Active', products: 2, updated: 'Just now' }],
      counts: { all: 1, tier: 1, bundle: 0, special: 0, inactive: 0 },
    };
  ...
  it('fetchDiscount requests /api/discounts/:id', async () => {
    const f = vi.fn().mockResolvedValue(jsonResponse({ discount: { id: 'abc', name: 'A', symbol: '%', type: 'Tier', status: 'Active', products: 0, updated: '—' }, campaign: null }));
