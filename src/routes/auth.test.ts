...
function createMockDb(options: { existingShop?: boolean } = {}) {
  const getMock = vi.fn().mockResolvedValue(
    options.existingShop ? { id: 'existing-shop' } : null
  );
...
describe('/shopify/install', () => {
  ...
  it('redirects to app when shop is already installed', async () => {
    vi.mocked(createDb).mockReturnValue(createMockDb({ existingShop: true }) as any);
    const env = createMockEnv({ existingShop: true });
    const res = await authRoutes.request('/shopify/install?shop=test.myshopify.com', {}, env);
    expect(res.status).toBe(302);
    expect(res.headers.get('location')).toBe('/?shop=test.myshopify.com');
  });

  it('redirects to Shopify OAuth URL for a new shop', async () => {
    vi.mocked(createDb).mockReturnValue(createMockDb({ existingShop: false }) as any);
    const env = createMockEnv({ existingShop: false });
    const mockResponse = new Response(null, {
      status: 302,
      headers: { location: 'https://test.myshopify.com/admin/oauth/authorize?client_id=test-client-id' },
    });
...
describe('/shopify/callback', () => {
  ...
  it('calls install lifecycle and redirects to app on success', async () => {
    ...
    const res = await authRoutes.request('/shopify/callback?code=authcode&shop=test.myshopify.com', {}, env);
    expect(res.status).toBe(302);
    expect(res.headers.get('location')).toBe('/?shop=test.myshopify.com');
    expect(mockSessionStorage.storeSession).toHaveBeenCalledWith(session);
    expect(onShopInstall).toHaveBeenCalledWith(
      'test.myshopify.com',
      { shop: 'test.myshopify.com', accessToken: 'token-abc', id: 'session-123' },
      env
    );
  });
});
